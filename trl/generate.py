...
class Generator(object):
    ...
    def encode_prefix(self, keywords, genres):
        ...
        for keyword, genre in zip(keywords, genres):
            ...
            tokenized_prefix = self.tokenizer(prefix)['input_ids'][0][:-1]
            lengths.append(len(tokenized_prefix))
            if len(tokenized_prefix) > max_length:
                assert False
            else:
                tokenized_prefix += [self.pad_id] * (max_length - len(tokenized_prefix))
            prefixes.append(tokenized_prefix)
        return prefixes
